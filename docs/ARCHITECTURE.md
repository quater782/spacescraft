...
## 友方 AI 动态协作（2026-09-25）

`game.js` 内 `aiPilotPartner` 对称解析队友，`aiHazardForecast` 每次决策只建立一份共享预测，`chooseAiStrategicGoal` 比较清敌、保护、救援、任务、星链与四类补给的效用，`aiNavigate` 在可达方向里避险。`aiHazardSeparation` 检查相对运动轨迹而非仅比较未来落点，避免高速弹/移动激光穿越采样间隙；曲线、制动再加速、种雷和限角追踪各自预测。普通决策间隔为模拟时间 0.1 秒，受伤、队友倒地和目标死亡可提前刷新。

高层目标保留对象引用和小幅连续性加分；收益、任务期限、队友覆盖与路线危险实时改变排序，闪避期间保留原目标，完成即释放。不进入永久 profile，不使用新随机流、网络或 AI 专属战斗特权。详细规则见[成长与协作平衡](./BALANCE-REBASE.md)。

双 AI 首章和平衡对照脚本只把双机接入正式控制器，不替换内部队友关系。`data-ai-intent` 为当前动作，`data-ai-goals` 为两机战略目标和前三候选得分，`data-ai-navigation` 为目标 ID 和路线风险，`data-ai-threats` 记录当前或被避开的危险。所有测试共用生产控制器。
...
`renderer3d.js` 从真实弹种、载荷、引信和制导时限选择像素轮廓；`projectile-art.js` 将逐格图形合并成带厚度与离散面色的共享三维网格，用实例绘制全部敌我弹头。弹体位置和方向直接来自世界快照，装饰旋转为 12 帧采样，游戏模拟和碰撞仍完全归 `game.js`。普通弹头没有圆润镜片或发光壳，少数特殊弹使用短原生线拖迹；曲面火焰和激光表现层保持独立。


## 像素实体与模块表现层（2026-09-10）

`renderer3d.js` 持有船体安装位与模块装配表现；`render-surfaces.js` 只持有像素实体/燃烧几何、材质、实例与线缓冲区。后者不读取输入、存档或战斗随机数，模拟时钟由前者传入。玩家机身及五类敌军设备采用硬边阶梯几何与四套三阶 Toon 色带，座舱/眼核为十九格像素体，实体环无倒角，尾焰为有时间形变的旋转曲面，瞄准和拖迹为原生线，激光为连续圆截面段。所有现有规则继续归 `game.js` 所有；没有新外部依赖或打包过滤例外。
