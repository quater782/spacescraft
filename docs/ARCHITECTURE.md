...
| 远征规则 | `src/expedition.js` | 9 种生态、种子航线、敌军船体权重与移动/武器/核心模块组装 |
...
战役开始时，机体、模组、合约、模式与设置从 profile 生成运行时玩家参数。同一枚 run seed 先固定三章生态航线，再驱动敌军船体、移动/武器/核心模块、弹幕、掉落与三选一候选。生态对象合并进运行时章节，向渲染器提供光色和 3D 地标，向模拟提供耐久/密度/弹速与敌型偏好，向音序器提供 BPM 和调式偏移。章节结束进入方向控制的共享构筑；星空、粒子和随机音色使用独立表现随机源，不会扰动后续玩法序列。任务结束后只把长期生涯、星尘、解锁、合约与设置写回存档，生态与局内升级留在本次航行。
...
`#game` Canvas 的 `data-*` 属性公开模式、语言、章节、事件、Boss 阶段、敌人数、玩家耐久、FPS、QA 状态、装备、合约、倍率、构筑、生态航线、已发现敌军变体、当前模块组合、HUD 分辨率和奖励等稳定摘要。本地参数 `qa-fast`、`qa-wallet`、`qa-contracts`、`qa-draft` 与 `seed` 用于复现测试；会改变进度或钱包的开关只在 localhost/127.0.0.1 生效。
