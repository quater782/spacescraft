...
- `dist/SPACECRAFT-0.15.0-windows-x64.zip`

0.15.0 Windows x64 校验值：
...
ZIP 为 157,775,417 字节，EXE 为 244,440,576 字节，解压后内容为 384,619,382 字节，共 76 个归档条目。ZIP 内部根目录使用 ASCII `SPACECRAFT-win32-x64`，避免跨平台解压工具误判中文文件夹编码；归档不包含 `__MACOSX` 或 `.DS_Store`，`unzip -tq` 完整性检查通过。

`app.asar` 为 1,080,366 字节，已核对为 0.15.0。它包含 Three.js `three.module.min.js`、Three.js 实例化体素渲染器、4 Buff/3 Debuff 状态模块、五槽 1,024 构筑、15 项局内升级、9 项天赋、7 项遗物协议、星链狂潮、9 种生态、7 种星门协议、5 类动态遭遇、真实 Electron 冒烟和全部验证脚本。未使用的 Three.js 源码、示例与非运行构建被 Forge 忽略，只保留 365,552 字节的运行模块；该模块 SHA-256 为 `86bcee248b64f44bcfc23c331ae74619061957d59cab040171dcb6fb5900beb6`。
