...
## 2026-09-25 低容量护盾实战检查

运行 `node_modules/.bin/electron scripts/smoke-shield-gameplay.cjs`。隔离 localhost 夹具让普通弹实际飞行至碰撞，使用正式 `update(1/60)` 推进事件，不手动设置受击年龄；以同帧中性外缘为参照，测量正式 `PixelEffects` GPU 缓冲的投影位移。覆盖低/平衡/高三档、3/4 点总容量、1/2 点伤害、最后一点盾受到 1/4 点攻击，以及吸收、溢出和事件回收。不得仅以数学响应函数非零或高护盾演示代替此检查。输出系统临时目录 `spacescraft-shield-gameplay/metrics.json`；这是受控回归，不是自然难度或真人体验证据。

`npm run smoke:effects -- --visual-only` 保留实机画面和正常首章 20 秒；同帧三向夹具使用 4 点盾与 1/2/3 点攻击，断言每发分别保存 0.25/0.5/0.75 负载，扣伤和吸收仍只计第一发。`--fixtures-only` 只在正常开局已检查后用于局部复验。

## 场景整合检查（2026-09-25）

`npm run smoke:biomes -- --scene-audit` 覆盖九生态并交替三档画质，检查场景 ID/质量接线、WebGL、帧率及零表面/粒子丢弃。单图复核可加 `--biome=sugarBloom`；`--motion-audit` 额外保存八帧远景动态，检查相位推进、暂停冻结及动画期间的零丢弃。`--motion-video` 同时录制轨道站、彗潮行星、裂环的正常速度片段；必须看完整游戏尺寸下的可见运动，不能把相位非零当作动态效果已经足够明显。`smoke:anomalies` 复核九种异象与正常开局，`smoke:models` 复核机体画廊及正常首章 20 秒。场景配置与影响来源见[场景系统](./SCENE-SYSTEM.md)。护盾画廊通过 `renderer3D.environment.scenery.draw` 临时隐藏背景，不再调用已移除的旧地标入口。

## 2026-09-25 护盾外缘传导与多发叠加验收

`npm run smoke:effects -- --visual-only --field-gallery` 检查实际普通弹三向同帧接触：全部消弹、三个方向/颜色独立保留，但保护时间只允许一次扣伤和吸收计数；固定步年龄、12 次上限、到期回收及破盾重建清理一并检查。渲染夹具检查压缩、回弹、远侧延迟、满容量负载比四分之一负载的凹陷至少深 1.5 倍、正负相位抵消、叠加顺序无关与周期接缝连续。
...
新碎片批次另用真实 `PixelEffects` 对象检查亮点/碎片各一颗的独立绘制范围、4096 共享容量/第 4097 颗丢弃、下一帧清空后零残留。该检查不替代实际 WebGL 材质编译和截图。
...
`npm run smoke:effects -- --visual-only --pulse-motion` 检查三档主波、真实 Nova 半径、波批次、粒子容量、既有护盾/激光，再刷新检查正常首章 20 秒。`--pulse-motion` 输出临时目录 `spacescraft-particle-effects/pulse-000.png` 至 `pulse-035.png`，为 30 FPS、1.2 秒受控双机释放序列；专门人为保留 12 枚敌弹以观察遮挡，不代表清弹结算。正常开局会刷新移除所有展示替换。

`--visual-only` 明确跳过独立声音/静音检查；本轮原路径在宿主音频时钟上未完成 80ms 音量渐变断言，失败值为 0.65，未把该音频检查写成通过。生产声音未改，视觉专项不依赖音频设备时钟。
...
## 2026-09-19 Nova 清弹范围验收

`npm run smoke:effects` 三档技能夹具分别截图释放后 0.04/0.18/0.42 秒，覆盖扩散和抵达范围后的渐淡；新增实际 Nova 事件半径与 `world.novaLastRadius` 一致性断言，保留双机事件及粒子零丢弃检查。默认命令随后刷新至正常首章并检查前 20 秒。完整章节、真人双人和 Windows 不由这些夹具覆盖。
...
用 `npm run smoke:projectiles` 复查种雷/爆破早晚引信、实际尺寸、高对比和低档截图，以及 192 弹体压力和正常 20 秒开局。该历史闭合环现已按用户要求移除，当前倒计时使用弹体蓄亮，Nova/爆炸用渐淡粒子波表现范围。
...
`npm run smoke:projectiles` 的种雷/爆破画廊现在包含爆炸半径，以检查引信边界；192 弹体场景同时要求实体与粒子零丢弃。`npm run smoke:effects -- --fixtures-only` 新增三档画质、0.02/0.1/0.2 秒的普通炮口与远端双节点截图，文件为临时目录 `spacescraft-particle-effects/hostile-muzzle-<quality>-<age>.png`。这些为受控视觉夹具，不代表自然战斗难度验收。

`npm run verify` 通过（生产依赖 0 漏洞）。特效专项三档炮口/远端节点采样与既有技能、护盾、激光夹具通过；弹体专项 20 种形态、192 弹体场景通过，60 FPS、零实例及粒子丢弃、零页面错误。正常首章 20.13 秒双机 7/7 HP、零倒地、60 FPS。已查看炮口与低档引信截图；完整首章、真人双人/双手柄及 Windows 未重验。

## 2026-09-19 像素粒子技能验收

`npm run smoke:effects -- --fixtures-only` 新增真实 `useNova` / `powerEffect` 事件截图：low、balanced、high 三档分别采样 0.04/0.18/0.42 秒，覆盖双机 Nova、炮口、命中、电弧和爆破，断言每档双 Nova 与零粒子丢弃。截图位于系统临时目录 `spacescraft-particle-effects/pixel-skills-<quality>-<age>.png`。初次夹具使用 100 充能未达到实际 120 阈值，已改为读取正式 `SpaceRush.NOVA_CONFIG.threshold` 后通过，不修改玩法门槛。

本轮另执行 `npm run smoke:projectiles`：20 弹体形态、高对比/低画质、192 弹体密集场景、正常首章 20 秒均通过；密集场景与开局均 60 FPS，双机 7/7 HP、零倒地和页面错误。已查看实际尺寸弹头以及高/低画质技能截图；这是视觉与短开局回归，不是完整首章难度或真人双人验收。

## 2026-09-19 智能芯片回归

`npm run smoke:enemy-ai` 在隔离 Electron/WebGL 中覆盖 16 船体和三章 Boss；新增四种芯片船体持续来弹 40 秒的真实发弹计数与闪避退出检查，保留自然生成保护/上限、中英扫描、低画质和单人/本地双人正常前 20 秒。`node scripts/verify-enemy-ai.mjs` 增加残血受压后撤、侧翼可达射位、机动提升与承诺不被闪避打断的检查。

首章芯片只在 142.5 秒后开放，正常速度抽样使用 `npm run smoke:chapter-one -- --dual-ai --offense --sample-seconds=210`，不能用前 20 秒的开局结果替代芯片阶段难度，也不能用短样本宣称完整首章通过。当前参数与诊断格式见[敌军规则](./ENEMY-AI-DESIGN.md)。
...
报告保存至系统临时目录 `spacescraft-chapter-one-dual-ai-offense/report.json`。这是控制器对照，不是正式双 AI 模式或真人难度证明；`fullChapterVerified:false` 的短样本不代表首章通过。默认脚本仍为 P1 跟随移动＋P2 正式 AI。时间上限不加速仿真、不补血，不把没死或击杀变多自动解释为数值平衡已完成。

2026-09-11 已执行：268.77 秒全灭，连接 260.53 秒，P1/P2 倒地 2/1 次、被救起 1/0 次、倒地 3.65/0.02 秒；155 击杀、60 FPS、零控制台错误。双机累计运行正式 AI 16387/16293 次。P1 220.88 秒首次倒地，222.55 秒被救起，266.78 秒再次倒地，P2 268.75 秒倒地。说明该样本不是长期缺席队员导致的持续单机输出，不证明总体游戏难度；星链增强的下一轮提案另见[任务型成长](./STARLINK-MASTERY-DESIGN.md)。

## 我方强度与爆破专项（机制通过，平衡未通过）

[我方强度重设计](./PLAYER-POWER-DESIGN.md)第 9 节为完整验收目标，第 11 节记录当前实现与缺口。运行 `node_modules/.bin/electron scripts/smoke-power.cjs`：仅监听 127.0.0.1，隔离存档和测试桥，直接调用真实碰撞、发弹、升级及章节函数。覆盖穿透衰减/追踪首碰结束、爆炸双机及内外边界/引信/防重复、节点 3 次拦截/失联暂停、6 秒过载每机 6/8 脉冲、独立副炮不被主炮超频影响、双模式三章继承、掉落实际拾取→发射→命中和中英卡面。输出位于系统临时目录 `spacescraft-power`，含 `mechanics.json`、四档弹体截图及中英选卡截图。夹具不是正常难度证据。

`npm run smoke:chapter-one` 为正常速度、方向输入和自然选卡长测；2026-09-11 生存优先样本在章节 251.25 秒全灭，完整首章验收失败（142 击杀、34 次追踪弹命中，60 FPS）。`npm run smoke:chapter-one -- --offense --sample-seconds=270` 使用同种子火力优先选卡，最多观察 270 秒；该观察允许记录死亡，输出明确标记 `fullChapterVerified:false`，不能当作完整首章通过。默认/对照目录分别是系统临时目录 `spacescraft-chapter-one-soak` / `spacescraft-chapter-one-offense`，后续执行会保存 `report.json` 和最终截图，包括失败样本。两种选卡策略不改变敌方配置，不使用 QA 加速、无敌或额外成长。

本轮应用内浏览器后端无法连接，使用真实 Electron/WebGL 后备路径。掉落两次封顶、短期增益弱和外观不联动仍为已知问题；先测试，不继续削弱敌方策略。完整三章、真人理解度和 Windows 未验证。

2026-09-11 火力对照已执行：同种子两次选择重炮，243.08 秒全灭；141 击杀，重炮 47/284 发命中、主炮 490/2689 发命中，60 FPS、零控制台错误。报告 `fullChapterVerified:false` / `survived:false`，不视为难度验收通过。机制专项也可通过 `npm run smoke:power` 运行；候选静态检查现覆盖全部十四选、合法前置和同级重复应用幂等。旧 `smoke:survival` 八选样本已改为两套新配方协议，全部强化样本仍为七套；本轮其夹具只更新，未额外重跑，继承由新专项覆盖。

最终检查记录：运行完整 `npm run verify`；本地构筑/双语等通过后，碰撞静态断言因新增受击位置参数与旧匹配式不兼容而中断。保留运行时的受击位置快照，修正匹配式后，碰撞至渲染器的剩余七项逐项复验通过。较早完整运行的在线审计已因 `ENOTFOUND registry.npmjs.org` 失败，本轮没有反复重试网络；不将完整命令描述为通过。工作区同时出现的像素特效模块和检查脚本均保留，没有回滚其修改。
